...
# 应用函数到每一行并生成描述性字符串
    ...
    np.savetxt('/home/cyyuan/ACL2025/Data/ACS/responses/acs_income30zero3.csv', data, delimiter=',', header='responses,gts', comments='', fmt='%s')  
    ...
    if responses.shape[0] != gts.shape[0]:  
        raise ValueError("responses 和 gts 数组的长度必须相同")  
    data = np.column_stack((responses, gts))  
    np.savetxt('/home/cyyuan/ACL2025/Data/ACS/responses/acs_income31zero3.csv', data, delimiter=',', header='responses,gts', comments='', fmt='%s')  
    print("complete writing data into acs_income.csv")
