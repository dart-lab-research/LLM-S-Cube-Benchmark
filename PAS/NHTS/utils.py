...
def load_data_NHTS(config):
    data_root_folder = config['data_root_folder']
    # trip_entire_df = pd.read_csv(os.path.join(data_root_folder, config['trip_entire_csv']))
    # hh_prompt_df = pd.read_csv(os.path.join(data_root_folder,config['hh_prompt_csv']))
    trip_entire_df = pd.read_csv(Path(data_root_folder) / config['trip_entire_csv'])
    hh_prompt_df = pd.read_csv(Path(data_root_folder) / config['hh_prompt_csv'])
...
def create_data_to_add_NHTS(id_fields, pred_fields, id_values, pred_values, pred_results):
    data_to_add = {}

    for field in id_fields:
        value = id_values.get(field.lower())  # 关键：转为小写匹配 key
        data_to_add[field] = value

    for field in pred_fields:
        gt_value = pred_values.get(field.lower())
        pred_value = pred_results.get(f"{field.lower()}_prediction")
        data_to_add[f"{field}_ground_truth"] = gt_value
        data_to_add[f"{field}_prediction"] = pred_value
