...
Question 2: What could be the most likely effect of basic income on your work choices?
The individual's answer is {question_bbi_2016wave4_basicincome_effect}
'''
...
def generate_questions(row):
    ...
    que = f'''which of the following arguments FOR the basic income do you find convincing?
'''
    prediction_task = f'''
    ...
    Your response should consist of just the option (1/2/3/4/5/6/7) to reflect the person's opinion, without any additional text, explanation or even a space letter. You are not allowed to explain anything of your response. Your entire output should not exceed 1 character.
...
Their responses to the survey questions are as follows:
Question 1: How familiar are you with the concept known as "basic income"
The individual's answer is The respondent knows something about the concept of 'basic income'.
additional information about basic income: "A basic income is an income unconditionally paid by the government to every individual regardless of whether they work and irrespective of any other sources of income. It replaces other social security payments and is high enough to cover all basic needs (food, housing etc.)."

Question 2: Which of the following arguments FOR the basic income do you find convincing?
The individual's answer is The respondent does not find any of the above arguments convincing for basic income.

Question 3: What could be the most likely effect of basic income on your work choices?
The individual's answer is The respondent does not select any of the above options regarding the impact of basic income on their work choices.

The answer is:  7.0


2.  This person, identified by 83127080-da3d-0133-c74f-0a81e8b09a82, is a 32-year-old male., This person lives in an urban area. None, This person does not have a full-time job., and There are no children in the individual's current household.. 
Their responses to the survey questions are as follows:
Question 1: How familiar are you with the concept known as "basic income"
The individual's answer is The respondent has heard just a little about the concept of 'basic income'.
additional information about basic income: "A basic income is an income unconditionally paid by the government to every individual regardless of whether they work and irrespective of any other sources of income. It replaces other social security payments and is high enough to cover all basic needs (food, housing etc.)."

Question 2: Which of the following arguments FOR the basic income do you find convincing?
The individual's answer is The respondent finds this argument convincing: basic income creates more equality of opportunity.

Question 3: What could be the most likely effect of basic income on your work choices?
The individual's answer is None
...
# 应用函数到每一行并生成描述性字符串
    ...
    #             continue
    #         gt = float(gt)
    #         # print(f"{i+1}. ",baseinfo+part_response)
    #         # print("The answer is: ", gt)
    ...
    # np.savetxt('/home/cyyuan/ACL2025/Data/Income/duolungpt/Argfor_round2_zero.csv', data, delimiter=',', header='gpt35,gpt4,gts', comments='', fmt='%s')  
    ...
    if responses1.shape[0] != gts.shape[0] or responses2.shape[0]!=gts.shape[0]:  
        raise ValueError("responses 和 gts 数组的长度必须相同")  
    data = np.column_stack((responses1, responses2, gts))  
    np.savetxt('/home/cyyuan/ACL2025/Data/Income/duolungpt/Argfor_round2_few.csv', data, delimiter=',', header='gpt35,gpt4,gts', comments='', fmt='%s')  
...
#round3
    ...
    if responses1.shape[0] != gts.shape[0] or responses2.shape[0]!=gts.shape[0]:  
        raise ValueError("responses 和 gts 数组的长度必须相同")  
    data = np.column_stack((responses1, responses2, gts))  
    np.savetxt('/home/cyyuan/ACL2025/Data/Income/duolungpt/Argfor_round3_zero.csv', data, delimiter=',', header='gpt35,gpt4,gts', comments='', fmt='%s')  
    print("complete writing data into zero.csv")
    ...
    if responses1.shape[0] != gts.shape[0] or responses2.shape[0]!=gts.shape[0]:  
        raise ValueError("responses 和 gts 数组的长度必须相同")  
    data = np.column_stack((responses1, responses2, gts))  
    np.savetxt('/home/cyyuan/ACL2025/Data/Income/duolungpt/Argfor_round3_few.csv', data, delimiter=',', header='gpt35,gpt4,gts', comments='', fmt='%s')  
    print("complete writing data into few.csv")
