...
def main():
    ...
    input_path = r'D:\School\UM\LLM\Data\Trell social media usage\train_age_dataset.csv'
    log_path = r'D:\School\UM\LLM\Media\response_gpt\log.txt'
    responses1 = []
    responses2 = []
    preds = []
    ...
    for round in range(2):
        with open(input_path, mode='r', newline='') as infile:
            ...
            for i,row in enumerate(reader):
                ...
                with open(r"D:\School\UM\LLM\Media\response_gpt\prompt.txt","a") as prompt_file:
                    prompt_file.write(prompt+"\n")
                #print(prompt)
                conversation_history = [{"role": "user", "content": prompt}]
                try:  
                    #print(temp)
                    response1 = ask_gpt4(client, conversation_history)
                    #print(response1)
                    with open(r"D:\School\UM\LLM\Media\response_gpt\response.txt", "a") as respones_file:
                        respones_file.write(str(response1) + "\n")
                ...
                match = re.search(r'[A-F]', response1)
                if match:
                    response1 = match.group()
                else:
                    response1 = response1.strip()  # 如果没有找到A-F，就去除空白字符
                print(response1)
                cnt=cnt+1
                if response1 == pred:
                    correct1 = correct1+1
                # if response2 == pred:
                #     correct2 = correct2+1
                print(f"No:{i}: {correct1}  answer:{pred} response{response1}")
        ...
        with open('D:/School/UM/LLM/Media/response_gpt/acc.txt', 'a') as file:  # 使用 'a' 模式可以追加内容
            file.write(f"[{time_str}] weekdays_round{round+1}_zero_gpt35:  {acc1}%\n")
