...
<<<<<<<<<<<<<<  ✨ Windsurf Command ⭐ >>>>>>>>>>>>>>>>
        """
        Initialize a LLMHandler with the given LLM model name and API keys.

        If `use_api` is True, the LLMHandler will use the API to communicate with the
        model. Otherwise, it will use a local model.
        ...
        api_keys : dict
            A dictionary containing the API keys for the LLM model.
        use_api : bool, optional
            Whether to use the API to communicate with the model, by default False.

        Raises
        ------
        ValueError
            If the LLM model is not supported locally and `use_api` is False.
        """
<<<<<<<  cc9a3ace-7667-4039-9c1e-3a03bf4b7eeb  >>>>>>>>
        self.llm = llm
