...
# 应用函数到每一行并生成描述性字符串
    ...
    np.savetxt('/home/cyyuan/ACL2025/Data/Anes2020/duolun/TRUMP_llama_zero.csv', data, delimiter=',', header='llama3,llama31,gts', comments='', fmt='%s')  
    print("complete writing data into trump.csv")
    ...
    with open(input_path, mode='r', newline='') as infile:
        ...
        for i, row in enumerate(reader):
            ...
            prompt = base_description + assess_description +roleplayinfo + question+few_examples
            conversation_history = [{"role": "user", "content": prompt}]
    ...
    if responses1.shape[0] != gts.shape[0] or responses2.shape[0]!=gts.shape[0]:  
        raise ValueError("responses 和 gts 数组的长度必须相同")  
    data = np.column_stack((responses1,responses2, gts))  
    np.savetxt('/home/cyyuan/ACL2025/Data/Anes2020/duolun/TRUMP_llama_few.csv', data, delimiter=',', header='llama3,llama31,gts', comments='', fmt='%s')  
    print("complete writing data into trump.csv")
